...
export default function Sidebar({ activePage, onNavigate, collapsed, onLogout }: Props) {
  ...
  const activeSection = navSections.find(s => s.items.some(i => i.id === activePage || i.children?.some(c => c.id === activePage)));

  return (
    <aside
      className="flex flex-col h-full bg-slate-900 transition-all duration-200"
      style={{ width: collapsed ? 56 : 240 }}
    >
      {/* Logo */}
      <div className="flex items-center gap-3 px-4 py-4 border-b border-slate-800 flex-shrink-0">
        <div className="w-8 h-8 bg-blue-600 rounded-lg flex items-center justify-center flex-shrink-0">
          <Icons.Cpu size={16} className="text-white" />
        </div>
        {!collapsed && (
          <div>
            <p className="text-white font-bold text-sm leading-none" style={{ fontFamily: "var(--font-display)" }}>MernCrest</p>
            <p className="text-slate-400 text-[10px] mt-0.5">IT Services</p>
          </div>
        )}
      </div>
      ...
      {!collapsed && (
        <div className="px-3 py-2 border-b border-slate-800 flex-shrink-0">
          <button className="w-full flex items-center justify-between bg-slate-800 hover:bg-slate-700 rounded-lg px-3 py-2 transition-colors">
            <div className="flex items-center gap-2">
              <Icons.MapPin size={12} className="text-slate-400" />
              <span className="text-xs text-slate-300">Colombo HQ</span>
            </div>
          ...
          return (
            <div key={sectionKey} className="mb-1">
              {!collapsed && (
                ...
                >
                  <span className="text-[10px] font-semibold text-slate-500 uppercase tracking-wider">{section.title}</span>
                  {isSectionCollapsed
                    ? <Icons.ChevronRight size={10} className="text-slate-600" />
                    : <Icons.ChevronDown size={10} className="text-slate-600 opacity-0 group-hover:opacity-100" />
                  }
                </button>
              )}
              {!isSectionCollapsed && (
                    ...
                    return (
                      <div key={item.id}>
                        ...
                        {!collapsed && hasChildren && isExpanded && (
                          <div className="ml-5 mt-0.5 space-y-0.5 pl-3 border-l border-slate-800">
        ...
        <div className="flex items-center gap-2.5">
          ...
          {!collapsed && (
            <div className="flex-1 min-w-0">
              <p className="text-slate-200 text-xs font-semibold truncate">Priya Jayawardena</p>
              <p className="text-slate-500 text-[10px] truncate">Operations Director</p>
            </div>
          )}
          {!collapsed && (
            <button onClick={onLogout} title="Sign out"
              className="text-slate-500 hover:text-red-400 transition-colors flex-shrink-0 p-1 rounded-lg hover:bg-red-400/10">
